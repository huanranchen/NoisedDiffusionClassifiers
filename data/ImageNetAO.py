...
from data import calibration_tools


def create_symlinks_to_imagenet(imagenet_folder, folder_to_scan,
                                PATH_TO_IMAGENET_VAL="/cephfs-thu/LargeData/ImageNet/val/"):
...
def get_imagenet_ao_loader(
    ...
    nae_loader = torch.utils.data.DataLoader(naes, batch_size=64, shuffle=False, num_workers=4, pin_memory=True)

    noes = dset.ImageFolder(root=PATH_TO_IMAGENET_O, transform=test_transform)
    noe_loader = torch.utils.data.DataLoader(noes, batch_size=64, shuffle=False, num_workers=4, pin_memory=True)
    ...
    val_loader_imagenet_o = torch.utils.data.DataLoader(
        val_examples_imagenet_o, batch_size=128, shuffle=False, num_workers=4, pin_memory=True
    )
    ...
    val_imagenet_loader = torch.utils.data.DataLoader(
        val_imagenet, batch_size=128, shuffle=False, num_workers=4, pin_memory=True
    )
    concat = lambda x: np.concatenate(x, axis=0)
    return nae_loader, noe_loader, val_loader_imagenet_o



def get_imagenet_o_loader(
    PATH_TO_IMAGENET_O="./resources/imagenet-o",
    PATH_TO_IMAGENET_VAL="/cephfs-thu/LargeData/ImageNet/val/",
    mean=(0.485, 0.456, 0.406),
    std=(0.229, 0.224, 0.225),
    batch_size=1,
    ...
    noes = dset.ImageFolder(root=PATH_TO_IMAGENET_O, transform=test_transform)
    noe_loader = torch.utils.data.DataLoader(noes, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)
    ...
    val_loader_imagenet_o = torch.utils.data.DataLoader(
        val_examples_imagenet_o, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True
    )

    val_imagenet = dset.ImageFolder(root=PATH_TO_IMAGENET_VAL, transform=test_transform)
    val_imagenet_loader = torch.utils.data.DataLoader(
        val_imagenet, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True
    )

    return noe_loader, val_loader_imagenet_o, imagenet_o_mask
...
def get_imagenet_o_results(in_loader, out_loader, net, mask):
    ...
    print(aurocs, auprs, fprs)


def test_imagenet_a(nae_loader, imagenet_a_mask, net: nn.Module):
    net.cuda()
    net.eval()
...
def test_imagenet_o(val_loader_imagenet_o, noe_loader, imagenet_o_mask, net: nn.Module):
    net.cuda()
    net.eval()
