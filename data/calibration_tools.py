...
def tune_temp(logits, labels, binary_search=True, lower=0.2, upper=5.0, eps=0.0001):
    ...
    if binary_search:
        ...
        expr = sum((cx.Minimize(cx.log_sum_exp(logits[i, :] * t) - logits[i, labels[i]] * t) for i in range(set_size)))
...
def show_calibration_results(confidence, correct, method_name="Baseline"):
    ...
    print("\t\t\t\t" + method_name)
    print("RMS Calib Error (%): \t\t{:.2f}".format(100 * calib_err(confidence, correct, p="2")))
