...
def get_list_image(path: str, sort_key=lambda x: int(x[:-4]), return_tensor=True) -> List[Tensor]:
    result = []
    images = os.listdir(path)
    images = [i for i in images if i.endswith((".jpg", ".png", ".jpeg"))]
    images.sort(key=sort_key)
    for image in images:
        result.append(get_image(os.path.join(path, image), return_tensor=return_tensor))
    return result
