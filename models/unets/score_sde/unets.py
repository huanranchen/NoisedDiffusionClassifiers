...
def get_NCSNPP(grad_checkpoint=False, pretrained=True, **kwargs):
...
def get_NCSNv2(**kwargs):
    with open("./models/unets/score_sde/cifar10_ncsnv2.yml") as f:
        config = yaml.safe_load(f)
    config = dict2namespace(config)
    model = NCSNv2(config, **kwargs)
    return model
