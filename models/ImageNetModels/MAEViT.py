...
__all__ = ["MAEViT", "MAEViTClassifier"]
...
class MAEViT(VisionTransformer):
    ...
    def init_weights(self) -> None:
        ...
        pos_embed = build_2d_sincos_position_embedding(
            int(self.num_patches**0.5), self.pos_embed.shape[-1], cls_token=True
        )
...
class MAEPreprocessor(nn.Module):
    def __init__(self):
        super().__init__()
        self.mean = nn.Parameter(torch.tensor([103.53, 116.28, 123.675]).view(3, 1, 1))
        self.std = nn.Parameter(torch.tensor([57.375, 57.12, 58.395]).view(3, 1, 1))

    def forward(self, x):
        pre = transforms.Compose(
            [transforms.Resize((224, 224)), transforms.Normalize(mean=self.mean / 255, std=self.std / 255)]
        )
        return pre(x)


def get_vit_base_config():
    config = dict(
        arch="b",
        img_size=224,
        patch_size=16,
...
class MAEViTClassifier(nn.Module):
    """
    input should be 224x224. Finetune or linear prob.
    """
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.data_preprocessor = MAEPreprocessor()
        self.backbone = MAEViT(*args, **kwargs)
        self.backbone.mask_ratio = 0  # when inference as classifiers, no mask
        self.head = MAEHead()

    def forward(self, x):
        x = self.backbone.forward(x)[0].mean(1)
        x = self.head(x)
        return x
